...
def _resolve_settings() -> Settings:
    """Runtime indirection so tests can monkeypatch module-level factories."""

    return get_settings()


def _call_factory(factory: object, settings: Settings) -> object:
    """Support both production factories(settings) and zero-arg test doubles."""
...
def _resolve_retriever(settings: Settings = Depends(_resolve_settings)) -> Retriever:
    """Call the current module-level retriever factory at request time."""
...
def _resolve_answer_chain(settings: Settings = Depends(_resolve_settings)) -> RAGAnswerChain:
    """Call the current module-level answer-chain factory at request time."""
